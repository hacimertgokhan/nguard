...
async function main() {
    ...
    const responsibility = await confirm('Do you take full responsibility for these changes and understand the risks?');
    if (!responsibility) {
...
async function gatherConfiguration() {
    ...
    for (const route of routes) {
        const create = await confirm(`Create ${route.path}? ${route.default ? '(recommended)' : ''}`);
        if (create) {
...
async function createAuthFile(config) {
    const ext = config.useTypeScript ? 'ts' : 'js';
    const libDir = path_1.default.join(config.projectRoot, 'lib');
    const authFile = path_1.default.join(libDir, `auth.${ext}`);
    ...
    fs_1.default.writeFileSync(authFile, content);
    console.log(`  ✓ Created ${authFile}`);
}
async function createApiRoutes(config) {
    const apiAuthDir = path_1.default.join(config.projectRoot, config.appDir, 'api', 'auth');
    // Create directories
    if (!fs_1.default.existsSync(apiAuthDir)) {
        fs_1.default.mkdirSync(apiAuthDir, { recursive: true });
    }
    const ext = config.useTypeScript ? 'ts' : 'js';
    // Create each route
    for (const route of config.authRoutes) {
        const routeDir = path_1.default.join(apiAuthDir, route);
        const routeFile = path_1.default.join(routeDir, `route.${ext}`);
        ...
        fs_1.default.writeFileSync(routeFile, content);
        console.log(`  ✓ Created ${routeFile}`);
    }
}
async function createProxyConfig(config) {
    const proxyFile = path_1.default.join(config.projectRoot, 'proxy.ts');
    const content = generateProxyContent(config);
    fs_1.default.writeFileSync(proxyFile, content);
    console.log(`  ✓ Created ${proxyFile}`);
}
async function createEnvTemplate(config) {
    const envFile = path_1.default.join(config.projectRoot, '.env.local.example');
    const content = generateEnvContent(config);
    fs_1.default.writeFileSync(envFile, content);
    console.log(`  ✓ Created ${envFile}`);
}
async function updateTsConfig(config) {
    const tsConfigPath = path_1.default.join(config.projectRoot, 'tsconfig.json');
...
function generateAuthFileContent(config) {
...
function generateRouteContent(route, config) {
    const isTs = config.useTypeScript;
    const routes = {
...
function generateProxyContent(config) {
    const isTs = config.useTypeScript;
    return `/**
